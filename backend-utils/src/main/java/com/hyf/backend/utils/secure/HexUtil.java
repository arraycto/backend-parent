...
public final class HexUtil {
    ...
    private static final int MASK1 = 0xF0;
    private static final int MASK2 = 0x0F;
    private static final int MASK3 = 0x01;
    private static final int MINUS_ONE = -1;
    private static final int SIXTEEN = 16;
    private static final String X1 = "0x";
    private static final String X2 = "0X";
    ...
    public static boolean isHexNumber(String value) {
        ...
        if (value.startsWith(X1, index)
                || value.startsWith(X2, index)
    ...
    public static byte[] decodeHex(char[] hexData) {
        ...
        for (int i = 0, j = 0; j < len; i++) {
            int f = toDigit(hexData[j], j) << FOURTH;
            j++;
    ...
    public static String toUnicodeHex(int value) {
        ...
        if (len < FOURTH) {
            // 不足4位补0
            builder.append("0000", 0, FOURTH - len);
        }
        builder.append(hex);
    ...
    public static String toUnicodeHex(char ch) {
        return "\\u"
                + DIGITS_LOWER[(ch >> TWELVE) & FIFTEEN]
                + DIGITS_LOWER[(ch >> EIGHT) & FIFTEEN]
                + DIGITS_LOWER[(ch >> FOURTH) & FIFTEEN]
                + DIGITS_LOWER[(ch) & FIFTEEN];
    }
    ...
    private static char[] encodeHex(byte[] data, char[] toDigits) {
        ...
        for (int i = 0, j = 0; i < len; i++) {
            // 高位
            out[j++] = toDigits[(MASK1 & data[i]) >>> FOURTH];
            // 低位
            out[j++] = toDigits[MASK2 & data[i]];
        }
        return out;
    }
    ...
    private static int toDigit(char ch, int index) {
        int digit = Character.digit(ch, SIXTEEN);
        if (digit == MINUS_ONE) {
